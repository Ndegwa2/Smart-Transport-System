...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        console.log("Login successful for user:", email, "Role:", row.role);
        res.json({ success: true, message: 'Sign in successful!', token: token, name: row.name, role: row.role });
    } catch (err) {
        console.error(err.message);
...
app.post('/register', async (req, res) => {
    const { name, email, password, reg_role } = req.body;
    const role = reg_role;
